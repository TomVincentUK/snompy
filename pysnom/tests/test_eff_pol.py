...
class TestEffPol:
    models = pysnom.fdm.bulk, pysnom.fdm.multi, pysnom.pdm
    taylor_models = (pysnom.fdm.bulk,)

    # eff_pol

    @pytest.mark.parametrize("model", models)
    def test_approach_curve_decays(self, model, scalar_sample_bulk, scalar_AFM_params):
        alpha_eff = model.eff_pol(
            sample=scalar_sample_bulk,
            **scalar_AFM_params | dict(z_tip=np.linspace(1, 100, 32) * 1e-9)
        )
        assert (np.diff(np.abs(alpha_eff), axis=-1) < 0).all()

    @pytest.mark.parametrize("model", models)
    def test_broadcasting(self, model, vector_sample_bulk, vector_AFM_params):
        a_sum = 0
        for a in [vector_sample_bulk.eps_stack[0], *vector_AFM_params.values()]:
            a_sum = a_sum + np.asarray(a)
        target_shape = a_sum.shape

        alpha_eff = model.eff_pol(sample=vector_sample_bulk, **vector_AFM_params)
    ...
    def test_approach_curve_n_decays(
        self, model, scalar_sample_bulk, scalar_AFM_params, scalar_tapping_params
    ):
    ...
    def test_harmonics_decay(
        ...
        alpha_eff = model.eff_pol_n(
            sample=scalar_sample_bulk,
            **scalar_AFM_params | scalar_tapping_params | dict(n=np.arange(2, 10))
        )
        assert (np.diff(np.abs(alpha_eff), axis=-1) < 0).all()

    @pytest.mark.parametrize("model", models)
    def test_demod_broadcasting(
        self, model, vector_sample_bulk, vector_AFM_params, vector_tapping_params
    ):
        a_sum = 0
        for a in [
    ...
    def test_multi_same_as_bulk_for_single_interface(
        ...
        np.testing.assert_allclose(
            model.bulk.eff_pol_n(**params), model.multi.eff_pol_n(**params), rtol=1e-4
        )

    # Taylor series

    @pytest.mark.parametrize("model", taylor_models)
    ...
    @pytest.mark.parametrize("model", taylor_models)
    def test_refl_coef_qs_from_taylor(
